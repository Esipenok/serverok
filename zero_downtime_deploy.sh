...
docker run --rm \
  ...
  node:18-alpine \
  sh -c "cd /app && node src/server.js --test"
...
# Обновляем основной Docker Compose файл на сервере
echo "Обновление Docker Compose конфигурации..."
cp "$RELEASE_PATH/docker/docker-compose.zero-downtime.yml" /app/docker-compose.zero-downtime.yml
